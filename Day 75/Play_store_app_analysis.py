...
Original file is located at
    https://colab.research.google.com/drive/1PjLUrzb7On4-bVyqtH4ttl3pPGx76si6
...
df_apps_clean[df_apps_clean.Content_Rating=="For adults only"]
...
"""# Vertical Bar Chart - Competition(Number of apps)"""
...
category_numbers = df_apps_clean.groupby('Category').agg({"App": pd.Series.count})
df_category_merged = pd.merge(category_numbers, category_installs, on='Category', how='inner')
df_category_merged.head()
...
scatter = px.scatter(df_category_merged, 
            ...
            color='App')
scatter.update_layout(xaxis_title='Numbers of apps', 
...
df_freevspaid = df_apps_clean.groupby(["Category", "Type"], as_index=False).agg({"App":pd.Series.count})
df_freevspaid.head()
...
types_installs = df_apps_clean.groupby("Type").agg({"Installs": pd.Series.sum})
types_installs
...
fix = px.box(df_paid_apps,
             x="Category",
             y="Revenue_Estimate",
             title="How much can apps earn?")
fix.update_layout(xaxis_title='Category',
